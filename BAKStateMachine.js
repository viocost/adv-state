...
class StateMachine {
          ...
          return (...args) => {
            setImmediate(async () => {
              ...
              try {
                ...
                target.error = err;
                console.warn(
                  `${target.name}: Event handler "${prop}" thrown an exception: ${err}`
                );
  ...
  processEvent(eventName, eventArgs) {
    ...
    if (newState) {
      ...
      if (this.isInfo())
        console.log(
          `%c ${this.name}: State is now set to ${this.state}`,
  ...
  performActions(actions, context, eventName, eventArgs) {
    ...
    for (let action of actions) {
      ...
      action.call(this.obj, eventName, eventArgs, this);
    }
  }
  ...
  isInfo() {
    return (
      this.traceLevel === StateMachine.TraceLevel.DEBUG ||
      this.traceLevel === StateMachine.TraceLevel.INFO
    );
  }
